...
const STATUS_COLORS: Record<string, string> = {
  ...
  pending: 'bg-white/5 text-[#6B8A76]',
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            ...
            className="w-full bg-[#122018] border border-[#1E3829] rounded-lg pl-9 pr-3 py-2 text-sm text-[#F9FAFB] placeholder-[#6B8A76] focus:border-[#41A650] focus:outline-none"
          />
        </div>
        <select value={spaceId} onChange={(e) => setSpaceId(e.target.value)} className="bg-[#122018] border border-[#1E3829] rounded-lg px-3 py-2 text-sm text-[#C8D5CE] focus:border-[#41A650] focus:outline-none">
          <option value="">All spaces</option>
          {spaces.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
        <select value={contentType} onChange={(e) => setContentType(e.target.value)} className="bg-[#122018] border border-[#1E3829] rounded-lg px-3 py-2 text-sm text-[#C8D5CE] focus:border-[#41A650] focus:outline-none">
          <option value="">All types</option>
          {['lesson', 'tutorial', 'faq', 'reference', 'transcript', 'article', 'decision', 'note'].map((t) => (
            <option key={t} value={t}>{t}</option>
          ))}
        </select>
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="bg-[#122018] border border-[#1E3829] rounded-lg px-3 py-2 text-sm text-[#C8D5CE] focus:border-[#41A650] focus:outline-none">
          <option value="">All statuses</option>
          {['ready', 'processing', 'pending', 'error'].map((s) => (
            <option key={s} value={s}>{s}</option>
          ))}
        </select>
        <button onClick={load} className="p-2 text-[#6B8A76] hover:text-[#C8D5CE] hover:bg-white/5 rounded-lg transition-colors" title="Refresh">
          <RefreshCw size={14} />
        </button>
      </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[...Array(5)].map((_, i) => <div key={i} className="h-14 bg-[#122018] border border-[#1E3829] rounded-xl animate-pulse" />)}
        </div>
      ) : documents.length === 0 ? (
        ...
        <div className="bg-[#122018] border border-[#1E3829] rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {documents.map((doc) => (
                <tr key={doc.id} className="border-b border-[#1E3829]/50 last:border-0 hover:bg-white/2 transition-colors">
                  <td className="px-4 py-3 max-w-[200px]">
                    <div className="flex items-center gap-2">
                      <FileText size={12} className="text-[#6B8A76] shrink-0" />
                      <p className="text-[#C8D5CE] truncate">{fileName(doc)}</p>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-xs text-[#6B8A76]">{doc.difficulty_level || '—'}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-wrap max-w-[120px]">
                      {(doc.topics || []).slice(0, 2).map((t) => (
                        <span key={t} className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-[#6B8A76]">{t}</span>
                      ))}
                      {(doc.topics?.length || 0) > 2 && <span className="text-[10px] text-[#6B8A76]">+{(doc.topics?.length || 0) - 2}</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#6B8A76]">{doc.pages_count ?? '—'}</td>
                  <td className="px-4 py-3 text-xs text-[#6B8A76]">{doc.chunks_count?.toLocaleString() ?? '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${STATUS_COLORS[doc.status] || 'bg-white/5 text-[#6B8A76]'}`}>
                      {doc.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#6B8A76]">{new Date(doc.created_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    {canManage && (
                      <button
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#091410] border border-[#1E3829] rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <p className="text-xs text-[#6B8A76] mb-6">All chunks and embeddings for this document will be permanently deleted.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDeleteId(null)} className="flex-1 px-4 py-2 bg-white/5 text-[#C8D5CE] rounded-lg text-sm hover:bg-white/10 transition-colors"><X size={12} className="inline mr-1" />Cancel</button>
