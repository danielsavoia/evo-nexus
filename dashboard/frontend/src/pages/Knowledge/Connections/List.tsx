...
function StatusBadge({ status }: { status: KnowledgeConnection['status'] }) {
  if (status === 'ready')
    return <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-[#41A650]/10 text-[#85F2A0]"><CheckCircle size={10} /> ready</span>
  ...
  return <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-white/5 text-[#6B8A76]"><Wifi size={10} /> disconnected</span>
}
  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-20 bg-[#122018] border border-[#1E3829] rounded-xl animate-pulse" />
        ))}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {connections.length === 0 ? (
        <div className="text-center py-16 bg-[#122018] border border-[#1E3829] rounded-xl">
          <div className="w-16 h-16 rounded-2xl bg-[#41A650]/10 flex items-center justify-center mx-auto mb-4">
            <Wifi size={28} className="text-[#85F2A0]" />
          </div>
          <p className="text-[#F9FAFB] font-medium mb-1">No connections yet</p>
          <p className="text-[#6B8A76] text-sm mb-6">Bring your own Postgres with pgvector to get started.</p>
          {canManage && (
            <button
              onClick={() => setShowWizard(true)}
              className="px-5 py-2 bg-[#41A650] text-[#091410] rounded-lg text-sm font-medium hover:bg-[#41A650]/90 transition-colors"
            >
        ...
        <div className="space-y-2">
          {connections.map((c) => (
            <div
              key={c.id}
              className="bg-[#122018] border border-[#1E3829] rounded-xl p-4 hover:border-[#41A650]/30 transition-colors"
            >
              <div className="flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1 flex-wrap">
                    {c.host && (
                      <span className="text-xs text-[#6B8A76]">{c.host}{c.port ? `:${c.port}` : ''}</span>
                    )}
                    {c.database_name && (
                      <span className="text-xs text-[#6B8A76]">/{c.database_name}</span>
                    )}
                    {c.spaces_count != null && (
                      <span className="text-xs text-[#6B8A76]">{c.spaces_count} space{c.spaces_count !== 1 ? 's' : ''}</span>
                    )}
                    {c.chunks_count != null && (
                      <span className="text-xs text-[#6B8A76]">{c.chunks_count.toLocaleString()} chunks</span>
                    )}
                    {c.schema_version && (
                      <span className="text-xs text-[#6B8A76]">schema {c.schema_version}</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => navigate(`/knowledge/connections/${c.id}`)}
                    className="p-2 text-[#6B8A76] hover:text-[#C8D5CE] hover:bg-white/5 rounded-lg transition-colors"
                    title="View details"
                  >
                    <ArrowRight size={14} />
                  </button>
