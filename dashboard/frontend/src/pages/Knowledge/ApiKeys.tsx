  ...
  if (loading) {
    return (
      <div className="space-y-2">
        {[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-[#122018] border border-[#1E3829] rounded-xl animate-pulse" />)}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 bg-[#091410] border border-[#1E3829] rounded-lg px-3 py-2 text-sm text-[#85F2A0] font-mono break-all">
                  {createdToken}
                </code>
                <button
                  onClick={copyToken}
            ...
            <button onClick={() => setCreatedToken(null)} className="p-1.5 rounded-lg text-[#6B8A76] hover:text-[#C8D5CE] hover:bg-white/5 shrink-0"><X size={14} /></button>
          </div>
        </div>
      )}
      ...
      {keys.length === 0 ? (
        ...
        <div className="bg-[#122018] border border-[#1E3829] rounded-xl overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {keys.map((k) => (
                <tr key={k.id} className={`border-b border-[#1E3829]/50 last:border-0 hover:bg-white/2 transition-colors ${k.revoked ? 'opacity-50' : ''}`}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Key size={12} className="text-[#6B8A76]" />
                      <p className="text-[#C8D5CE] font-medium">{k.name}</p>
                    </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {k.scopes.map((s) => (
                        <span key={s} className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-[#6B8A76]">{s}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#6B8A76]">
            ...
            <div className="px-5 py-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#122018] border border-[#1E3829] rounded-lg px-3 py-2 text-sm text-[#F9FAFB] focus:border-[#41A650] focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-xs text-[#6B8A76] mb-2">Scopes</label>
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                        form.scopes.includes(s)
                          ...
                          : 'bg-white/5 text-[#6B8A76] border border-[#1E3829] hover:border-[#6B8A76]'
                      }`}
                    >
                      {s}
                    </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-[#6B8A76] mb-1">Rate limit / min</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs text-[#6B8A76] mb-1">Rate limit / day</label>
                  <input
                    type="number"
                ...
                <div className="col-span-2">
                  <label className="block text-xs text-[#6B8A76] mb-1">Expiration date (optional)</label>
                  <input
                    type="date"
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleCreate} disabled={saving || !form.name.trim()} className="flex-1 px-4 py-2 bg-[#41A650] text-[#091410] rounded-lg text-sm font-semibold hover:bg-[#41A650]/90 transition-colors disabled:opacity-50">
      ...
      {confirmRevokeId && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#091410] border border-[#1E3829] rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
            ...
            <p className="text-xs text-[#6B8A76] mb-6">Any application using this key will immediately lose access. This cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmRevokeId(null)} className="flex-1 px-4 py-2 bg-white/5 text-[#C8D5CE] rounded-lg text-sm hover:bg-white/10 transition-colors">Cancel</button>
