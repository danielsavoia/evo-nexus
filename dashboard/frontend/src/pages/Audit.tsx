...
function getActionStyle(action: string) {
  ...
  return key ? ACTION_STYLE[key] : { bg: 'rgba(102,112,133,0.15)', text: '#6B8A76', border: 'rgba(102,112,133,0.30)' }
}
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#F7F9F8]">{t('audit.title')}</h1>
            <p className="text-sm text-[#6B8A76]">{total} entries</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#122018] rounded-xl border border-[#1E3829] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-[#6B8A76]">Loading...</td>
              </tr>
            ) : entries.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-12 text-center text-[#6B8A76]">No audit entries</td>
              </tr>
            ) : (
              entries.map((e) => {
                const style = getActionStyle(e.action)
                return (
                  <tr key={e.id} className="border-b border-[#1E3829]/50 hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3 text-[#6B8A76] text-xs whitespace-nowrap" title={new Date(e.created_at).toLocaleString()}>
                      {timeAgo(e.created_at)}
                    </td>
                    <td className="px-4 py-3 text-[#F7F9F8] font-medium">{e.username || '--'}</td>
                    ...
                    <td className="px-4 py-3 text-[#6B8A76]">{e.resource || '--'}</td>
                    <td className="px-4 py-3 text-[#6B8A76] max-w-xs truncate text-xs">{e.detail || '--'}</td>
                    <td className="px-4 py-3 text-[#6B8A76] text-xs font-mono">{e.ip_address || '--'}</td>
                  </tr>
                )
              })
            )}
        ...
        <div className="flex items-center justify-between mt-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page <= 1}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-[#1E3829] text-[#F7F9F8] hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
