...
export default function AgentDetail() {
  ...
  if (!hasAgentAccess(name)) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center bg-[#091410] gap-4">
        <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-[#122018] border border-[#1E3829]">
          <Lock size={28} className="text-[#6B8A76]" />
        </div>
        <div className="text-center">
          <p className="text-[#F7F9F8] font-semibold text-base mb-1">Acesso restrito</p>
          <p className="text-[#6B8A76] text-sm">Você não tem permissão para acessar este agente.</p>
        </div>
        <Link
          to="/agents"
  ...
  if (loading) {
    return (
      <div className="h-full w-full flex items-center justify-center bg-[#091410]">
        <div className="text-[#6B8A76] text-xs uppercase tracking-[0.12em]">loading agent…</div>
      </div>
    )
  }
  ...
  if (!content) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center bg-[#091410] gap-3">
        <p className="text-[#6B8A76] text-sm">Agent not found</p>
  ...
  return (
    <div className="clever-app-shell flex h-full w-full flex-col">
      {/* ── HERO STRIP ─────────────────────────────────────────────── */}
      <header className="flex-shrink-0 h-20 flex items-center px-4 lg:px-6 gap-4 border-b border-[#1E3829] bg-[#07130D]/90 backdrop-blur-sm">
        <Link
          to="/agents"
          className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.14em] text-[#6B8A76] hover:text-[#F7F9F8] transition-colors"
        >
          <ArrowLeft size={12} />
          Agents
        </Link>
        ...
        <div className="flex flex-col gap-0.5 min-w-0">
          <h1 className="text-[16px] font-semibold text-[#F7F9F8] tracking-tight truncate">
            {formatName(name)}
          </h1>
          <code
        ...
        <div className="ml-auto flex items-center gap-4">
          <span className="hidden sm:inline text-[10px] uppercase tracking-[0.12em] text-[#6B8A76]">
          ...
          <button
            ...
            className="lg:hidden flex h-8 w-8 items-center justify-center rounded-md border border-[#1E3829] text-[#C8D5CE] hover:text-[#F7F9F8] hover:border-[#2E5040]"
      ...
      <div className="flex flex-1 min-h-0 relative">
        {/* Info rail — desktop */}
        <aside className="hidden lg:flex flex-col w-[320px] flex-shrink-0 border-r border-[#1E3829] bg-[#07130D]">
          <InfoRail
            tab={tab}
            setTab={setTab}
        ...
        {railOpen && (
          ...
          >
            <aside
              className="absolute top-14 left-0 bottom-0 w-[85vw] max-w-[340px] border-r border-[#1E3829] bg-[#07130D] flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between px-4 h-10 border-b border-[#1E3829]">
                <span className="text-[10px] uppercase tracking-[0.12em] text-[#6B8A76]">
                  {formatName(name)}
                </span>
                <button
                  onClick={() => setRailOpen(false)}
                  className="text-[#C8D5CE] hover:text-[#F7F9F8]"
                  aria-label="Close"
                >
                  <X size={14} />
                </button>
        ...
        {/* Terminal / Chat stage */}
        <section className="flex-1 min-w-0 relative overflow-hidden flex flex-col" style={{ background: 'linear-gradient(180deg, #091410 0%, #07130D 100%)' }}>
          {/* Ambient glow */}
          <div
          ...
          <div className="relative z-10 flex items-center flex-shrink-0 h-9 border-b border-[#1E3829] bg-[#07130D]">
            {/* View mode toggle */}
            ...
            {viewMode === 'terminal' && (
              <div className="flex items-center flex-1 h-full overflow-x-auto">
                {termTabs.length > 1 && termTabs.map((tt) => (
                  <div
                    key={tt.id}
                    className={`group flex items-center gap-2 px-3 h-full text-[11px] cursor-pointer border-r border-[#1E3829] transition-colors ${
                      activeTermTab === tt.id
                        ? 'bg-[#091410] text-[#F7F9F8]'
                        : 'text-[#C8D5CE] hover:text-[#F7F9F8] hover:bg-[#122018]'
                    }`}
                  ...
                  >
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 text-[#6B8A76] hover:text-[#ef4444] transition-opacity"
                    >
                      <X size={11} />
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center h-full px-2.5 text-[#6B8A76] hover:text-[#F7F9F8] hover:bg-[#122018] transition-colors"
                  title="New terminal"
                >
                  <Plus size={13} />
                </button>
...
function InfoRail({
  ...
  return (
    <>
      {/* Tab bar */}
      <div className="flex-shrink-0 flex items-center h-10 px-5 gap-6 border-b border-[#1E3829]">
        <TabButton
          label="Sessions"
          active={tab === 'sessions'}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {tab === 'profile' && (
          <div className="px-5 py-4">
            {profileLead && (
              <p className="text-[13px] leading-[1.6] text-[#F7F9F8] mb-4 pb-4 border-b border-[#1E3829]">
                {profileLead}
              </p>
            )}
            <div
              className="prose-agent text-[12.5px] leading-[1.65] text-[#C8D5CE]"
              style={
                {
        ...
        {tab === 'memory' && (
          <div className="px-5 py-4">
            {memories.length === 0 ? (
              <div className="text-[12px] text-[#6B8A76]">
                <p className="mb-1">Sem memórias ainda.</p>
                <p className="text-[11px] text-[#3D5445]">
                  Adicione arquivos em{' '}
                  <code className="font-mono text-[#6B8A76]">.claude/agent-memory/{agentSlug}/</code>
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <li key={mem.name}>
                      <button
                        ...
                        className="w-full flex items-center gap-2 py-1.5 px-2 rounded-md hover:bg-[#122018] text-left transition-colors"
                      >
                        {open ? (
                          <ChevronDown size={11} className="text-[#6B8A76] flex-shrink-0" />
                        ) : (
                          <ChevronRight size={11} className="text-[#3D5445] flex-shrink-0" />
                        )}
                        <span className="font-mono text-[11.5px] text-[#F7F9F8] truncate">
                          {mem.name}
                        </span>
                        <span className="ml-auto font-mono text-[10px] text-[#6B8A76] flex-shrink-0">
                          {formatSize(mem.size)}
                        </span>
                      </button>
                      {open && (
                        <div
                          className="ml-4 mt-1 mb-2 pl-4 py-1 border-l text-[11.5px] leading-[1.6] text-[#C8D5CE] overflow-hidden"
...
function TabButton({
  ...
  return (
    ...
    >
      {label}
      {count !== undefined && (
        <span className="font-mono text-[10px] text-[#3D5445]">{count}</span>
      )}
      {active && (
        <span
