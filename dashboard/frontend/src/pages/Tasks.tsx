...
const STATUS_STYLES: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  completed: { bg: 'bg-[#41A650]/10 border-[#41A650]/20', text: 'text-[#85F2A0]', dot: 'bg-[#41A650]' },
  failed: { bg: 'bg-red-500/10 border-red-500/20', text: 'text-red-400', dot: 'bg-red-400' },
  cancelled: { bg: 'bg-[#6B8A76]/10 border-[#6B8A76]/20', text: 'text-[#6B8A76]', dot: 'bg-[#6B8A76]' },
}
...
export default function Tasks() {
  ...
  if (loading) {
    return (
      <div>
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[#F7F9F8]">{t('tasks.title')}</h1>
        </div>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#122018] border border-[#1E3829] flex items-center justify-center">
            <CalendarClock size={20} className="text-[#85F2A0]" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-[#F7F9F8]">{t('tasks.title')}</h1>
            <p className="text-[#6B8A76] mt-0.5 text-sm">One-off scheduled actions</p>
          </div>
        </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors border ${
              filter === f.value
                ...
                : 'bg-[#122018] border-[#1E3829] text-[#6B8A76] hover:text-[#F7F9F8] hover:border-[#1E3829]'
            }`}
          >
            {f.label}
          </button>
        ))}
        <span className="text-[11px] text-[#6B8A76] ml-2">{total} tasks</span>
      </div>

      {/* Table */}
      {tasks.length === 0 ? (
        <div className="bg-[#122018] border border-[#1E3829] rounded-xl p-12 text-center">
          <CalendarClock size={32} className="text-[#6B8A76] mx-auto mb-3" />
          <p className="text-[#6B8A76] text-sm">No scheduled tasks yet</p>
          <button onClick={openCreate} className="mt-3 text-[#85F2A0] text-sm hover:underline">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {tasks.map((task) => {
                ...
                return (
                  <tr key={task.id} className="border-t border-[#1E3829]/50 hover:bg-white/[0.02] transition-colors">
                    <td className="p-4">
                      <div className="text-[#F7F9F8] font-medium">{task.name}</div>
                      {task.description && (
                        <div className="text-[#6B8A76] text-xs mt-0.5 truncate max-w-[300px]">{task.description}</div>
                      )}
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      {task.agent ? (
                        <span className="inline-flex text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-[#41A650]/8 border border-[#41A650]/20 text-[#85F2A0]">
                    ...
                    <td className="p-4 text-[#F7F9F8] text-xs font-mono">{formatDate(task.scheduled_at)}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1 justify-end">
                        {task.status === 'pending' && (
                          <>
                            <button onClick={() => handleRunNow(task.id)} className="p-1.5 rounded-lg hover:bg-[#41A650]/10 text-[#6B8A76] hover:text-[#85F2A0] transition-colors" title="Run Now">
                              <Play size={14} />
                            </button>
                            <button onClick={() => openEdit(task)} className="p-1.5 rounded-lg hover:bg-white/5 text-[#6B8A76] hover:text-[#F7F9F8] transition-colors" title="Edit">
                              <Pencil size={14} />
                            </button>
                            <button onClick={() => handleCancel(task.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-[#6B8A76] hover:text-red-400 transition-colors" title="Cancel">
                              <X size={14} />
                            </button>
                          </>
                        )}
                        {(task.status === 'completed' || task.status === 'failed') && (
                          <>
                            <button onClick={() => setViewTask(task)} className="p-1.5 rounded-lg hover:bg-white/5 text-[#6B8A76] hover:text-[#F7F9F8] transition-colors" title="View Result">
                              <Eye size={14} />
                            </button>
                            <button onClick={() => handleDelete(task.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-[#6B8A76] hover:text-red-400 transition-colors" title="Delete">
                              <Trash2 size={14} />
                            </button>
                          </>
                        )}
                        {task.status === 'failed' && (
                          <button onClick={() => handleRunNow(task.id)} className="p-1.5 rounded-lg hover:bg-[#41A650]/10 text-[#6B8A76] hover:text-[#85F2A0] transition-colors" title="Retry">
                            <RefreshCw size={14} />
                          </button>
                        )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-[#122018] border border-[#1E3829] rounded-xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1E3829]">
              <h2 className="text-lg font-semibold text-[#F7F9F8]">
                {editingTask ? 'Edit Task' : 'New Scheduled Task'}
              </h2>
              <button onClick={() => setShowModal(false)} className="p-1 rounded-lg hover:bg-white/5 text-[#6B8A76] hover:text-[#F7F9F8]">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B8A76] mb-1.5">Description</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-[#6B8A76] mb-1.5">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 bg-[#07130D] border border-[#1E3829] rounded-lg text-sm text-[#F7F9F8] placeholder-[#6B8A76] focus:border-[#41A650]/50 focus:outline-none font-mono resize-none"
                />
              </div>

              <div>
                <label className="block text-xs font-medium text-[#6B8A76] mb-1.5">Scheduled At *</label>
                <input
                  type="datetime-local"
      ...
      {viewTask && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setViewTask(null)}>
          <div className="bg-[#122018] border border-[#1E3829] rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1E3829]">
              <div>
                <h2 className="text-lg font-semibold text-[#F7F9F8]">{viewTask.name}</h2>
                <p className="text-xs text-[#6B8A76] mt-0.5">
              ...
              <button onClick={() => setViewTask(null)} className="p-1 rounded-lg hover:bg-white/5 text-[#6B8A76] hover:text-[#F7F9F8]">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#6B8A76] mb-1">Payload</label>
                <pre className="text-xs text-[#F7F9F8] bg-[#07130D] border border-[#1E3829] rounded-lg p-3 whitespace-pre-wrap font-mono">{viewTask.payload}</pre>
              </div>

              {viewTask.result_summary && (
                <div>
                  <label className="block text-xs font-medium text-[#85F2A0] mb-1">Result</label>
                  <pre className="text-xs text-[#F7F9F8] bg-[#07130D] border border-[#1E3829] rounded-lg p-3 whitespace-pre-wrap font-mono max-h-64 overflow-y-auto">{viewTask.result_summary}</pre>
                </div>
              )}
