    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#F7F9F8]">{t('roles.title')}</h1>
            <p className="text-sm text-[#6B8A76]">{roles.length} role{roles.length !== 1 ? 's' : ''} configured</p>
          </div>
        </div>
        {!isEditing && (
          <button
      ...
      {!isEditing && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
          {roles.map(role => {
            ...
            return (
              <div key={role.id} className="bg-[#122018] rounded-xl border border-[#1E3829] p-5 hover:border-[#41A650]/30 transition-all group">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-[#F7F9F8]">{role.name}</h3>
                    {role.is_builtin ? (
                      <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full border bg-[#1E3829]/60 border-[#1E3829] text-[#6B8A76]">
                ...
                <p className="text-xs text-[#6B8A76] mb-3">{role.description || 'No description'}</p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {Object.keys(role.permissions).length > 4 && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#1E3829] text-[#6B8A76]">
                        +{Object.keys(role.permissions).length - 4}
                      </span>
                    )}
                  </div>
                  <span className="text-[11px] font-medium text-[#6B8A76] bg-[#1E3829] px-2 py-0.5 rounded-full shrink-0 ml-2">
      ...
      {/* Permission Editor */}
      {isEditing && (
        <div className="bg-[#122018] rounded-xl border border-[#1E3829] p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-[#F7F9F8]">
              {creating ? 'Create New Role' : `Edit: ${editingRole?.name}`}
            </h2>
            <button onClick={closeEditor} className="text-[#6B8A76] hover:text-[#F7F9F8] transition-colors"><X size={18} /></button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <label className="block text-sm font-medium text-[#F7F9F8] mb-1">Name</label>
              <input
                type="text"
                value={newName}
                onChange={(e) => setNewName(e.target.value)}
                disabled={editingRole?.is_builtin}
                className="w-full px-3 py-2 rounded-lg bg-[#07130D] border border-[#1E3829] text-[#F7F9F8] text-sm focus:outline-none focus:border-[#41A650] disabled:opacity-50 transition-colors"
                placeholder="e.g. moderator"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#F7F9F8] mb-1">Description</label>
              <input
                type="text"
                value={newDesc}
          ...
          {/* Permissions matrix */}
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-[#F7F9F8] mb-3">Permissions</h3>
            <div className="overflow-x-auto rounded-lg border border-[#1E3829]">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[#6B8A76] text-xs uppercase tracking-wider bg-[#07130D]">
                ...
                <tbody>
                  {Object.entries(resources).map(([resource, availableActions]) => {
                    ...
                    return (
                      <tr key={resource} className="border-t border-[#1E3829]/50 hover:bg-white/[0.02] transition-colors">
                        <td className="py-2.5 pl-4 pr-4 text-[#F7F9F8] font-medium capitalize">{resource}</td>
                        {['view', 'execute', 'manage'].map(action => {
                                ...
                                <button
                                  ...
                                  className={`w-6 h-6 rounded border-2 flex items-center justify-center mx-auto transition-colors ${
                                    checked
                                      ...
                                      : 'border-[#1E3829] hover:border-[#6B8A76]'
                                  }`}
                                >
                                  {checked && <Check size={14} className="text-[#07130D]" />}
                                </button>
                              ) : (
                        ...
                        <td className="text-center py-2.5 px-3">
                          ...
                          >
                            {allChecked && <Check size={14} className="text-[#07130D]" />}
                          </button>
                        </td>
                      </tr>
                    )
          ...
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-[#F7F9F8] mb-3">Acesso a Agentes</h3>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg border text-sm transition-colors ${
                    editAgentAccess.mode === value
                      ...
                      : 'border-[#1E3829] text-[#6B8A76] hover:text-[#F7F9F8] hover:border-[#2E5040]'
                  }`}
                >
                  {label}
                </button>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-3 rounded-xl border transition-all ${
                        active
                          ? 'bg-[#41A650]/10 border-[#41A650]/40 text-[#85F2A0]'
                          : 'border-[#1E3829] text-[#6B8A76] hover:border-[#2E5040] hover:text-[#F7F9F8]'
                      }`}
                    >
            ...
            {editAgentAccess.mode === 'selected' && (
              <div className="rounded-lg border border-[#1E3829] overflow-hidden">
                {(['business', 'engineering'] as const).map((layer) => {
                  ...
                  return (
                    <div key={layer} className="border-b border-[#1E3829] last:border-b-0">
                      <div className="flex items-center justify-between px-4 py-2 bg-[#07130D]">
                        <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-[#6B8A76]">
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-left transition-colors ${
                                isSelected
                                  ? 'bg-[#41A650]/8 border-[#41A650]/20 text-[#F7F9F8]'
                                  : 'border-[#1E3829] text-[#6B8A76] hover:border-[#2E5040] hover:text-[#F7F9F8]'
                              }`}
                            >
                              <div
                                className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                                  isSelected ? 'bg-[#41A650] border-[#41A650]' : 'border-[#1E3829]'
                                }`}
                              >
                                {isSelected && <Check size={10} className="text-[#07130D]" />}
                              </div>
          ...
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-[#F7F9F8] mb-1 flex items-center gap-2">
              <FolderOpen size={14} className="text-[#85F2A0]" />
              Pastas do Workspace
            </h3>
            <p className="text-xs text-[#6B8A76] mb-3">Define quais pastas de primeiro nível do workspace este role pode acessar.</p>
