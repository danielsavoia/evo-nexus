...
| **`docker exec openclaude login` é plano B** | Aceito apenas para troubleshooting ou debug quando o OAuth pela UI falhar. Não é o fluxo preferido nem um procedimento normal de instalação. |
...
| **Endurecer/validar Login Codex OAuth pela UI em fresh install** | Alta | O fluxo oficial é Providers → OpenAI Codex OAuth → Login — já presente e validado em beta.14. Pendente: validação adicional em instalação totalmente limpa (volume `clever_agent_codex_auth` vazio + primeiro login OAuth completo pela UI), melhores mensagens de erro caso o OAuth falhe, e confirmação de persistência do volume `/root/.codex` entre redeploys. `docker exec <container> openclaude login` permanece apenas como plano B/debug caso o OAuth pela UI não funcione. |
